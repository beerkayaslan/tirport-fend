// auth
const LOGIN = '/login';
const REGISTER = '/register';

// routes
const INDEX = '/';

const DETAIL = 'detail/:id';
const EDIT = 'edit/:id';

const ADDRESS = 'address';
const COMPANY = 'company';
const DRIVER_INVENTORY = 'driver-inventory';
const PROJECT = 'project';
const USER_MANAGEMENT = 'user-management';
const VEHICLE_INVENTORY = 'vehicle-inventory';

export const ROUTES = {
  INDEX: { PATH: INDEX },
  LOGIN: { PATH: LOGIN },
  REGISTER: { PATH: REGISTER },

  ADDRESS: {
    PATH: ADDRESS,
    DETAIL: { PATH: `${ADDRESS}/${DETAIL}` },
    EDIT: { PATH: `${ADDRESS}/${EDIT}` }
  },

  COMPANY: {
    PATH: COMPANY,
    EDIT: { PATH: `${COMPANY}/${EDIT.replace(':id', '')}` }
  },

  DRIVER_INVENTORY: {
    PATH: DRIVER_INVENTORY,
    DETAIL: { PATH: `${DRIVER_INVENTORY}/${DETAIL}` },
    EDIT: { PATH: `${DRIVER_INVENTORY}/${EDIT}` }
  },

  PROJECT: {
    PATH: PROJECT,
    DETAIL: { PATH: `${PROJECT}/${DETAIL}` },
    EDIT: { PATH: `${PROJECT}/${EDIT}` }
  },

  USER_MANAGEMENT: {
    PATH: USER_MANAGEMENT,
    DETAIL: { PATH: `${USER_MANAGEMENT}/${DETAIL}` },
    EDIT: { PATH: `${USER_MANAGEMENT}/${EDIT}` }
  },

  VEHICLE_INVENTORY: {
    PATH: VEHICLE_INVENTORY,
    DETAIL: { PATH: `${VEHICLE_INVENTORY}/${DETAIL}` },
    EDIT: { PATH: `${VEHICLE_INVENTORY}/${EDIT}` }
  }
};

export const URLS = {
  INDEX,
  LOGIN,
  REGISTER,

  ADDRESS: `/${ADDRESS}`,
  ADDRESS_DETAIL: (id: string) => `/${ADDRESS}/${DETAIL.replace(':id', id)}`,
  ADDRESS_EDIT: (id: string) => `/${ADDRESS}/${EDIT.replace(':id', id)}`,

  COMPANY: `/${COMPANY}`,
  COMPANY_EDIT: `/${COMPANY}/${EDIT.replace('/:id', '')}`,

  DRIVER_INVENTORY: `/${DRIVER_INVENTORY}`,
  DRIVER_INVENTORY_DETAIL: (id: string) => `/${DRIVER_INVENTORY}/${DETAIL.replace(':id', id)}`,
  DRIVER_INVENTORY_EDIT: (id: string) => `/${DRIVER_INVENTORY}/${EDIT.replace(':id', id)}`,

  PROJECT: `/${PROJECT}`,
  PROJECT_DETAIL: (id: string) => `/${PROJECT}/${DETAIL.replace(':id', id)}`,
  PROJECT_EDIT: (id: string) => `/${PROJECT}/${EDIT.replace(':id', id)}`,

  USER: `/${USER_MANAGEMENT}`,
  USER_DETAIL: (id: string) => `/${USER_MANAGEMENT}/${DETAIL.replace(':id', id)}`,
  USER_EDIT: (id: string) => `/${USER_MANAGEMENT}/${EDIT.replace(':id', id)}`,

  VEHICLE_INVENTORY: `/${VEHICLE_INVENTORY}`,
  VEHICLE_INVENTORY_DETAIL: (id: string) => `/${VEHICLE_INVENTORY}/${DETAIL.replace(':id', id)}`,
  VEHICLE_INVENTORY_EDIT: (id: string) => `/${VEHICLE_INVENTORY}/${EDIT.replace(':id', id)}`
};
